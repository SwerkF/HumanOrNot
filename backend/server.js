...
const wss = new ws.Server({ port: process.env.PORT_WS });
    ...
    if (message.type === "message_human") {
      ...
      try {
        setTimeout(async () => {
          ...
          // Vérifier si la partie est terminée
          const isGameOver = game.messages.length === 10 ? true : false;
