...
echarts.use([BarChart, PieChart, CanvasRenderer, GridComponent, TooltipComponent, LegendComponent]);
...
export default function Stats() {
    const { games, error, status } = useAppSelector((state: RootState) => state.game);
    ...
    useEffect(() => {
        ...
        const gamesPerMonth = last6Months.map(month => ({
            month,
            count: games.filter((game: Game) => dayjs(game.createdAt).format('YYYY-MM') === month).length
        }));
        ...
        pieChartInstance.current.setOption({
            ...
            series: [{
                ...
                radius: '50%',
                data: [
                    { value: wins, name: 'Victoires', itemStyle: { color: colors.primary } },
                    { value: losses, name: 'Défaites', itemStyle: { color: colors.secondary } }
                ]
            }]
        });
    ...
    return (
        <main className="flex flex-col justify-center items-center gap-4 h-screen">
            ...
            <p className="text-white text-md">Voir les statistiques sur vos parties jouées.</p>
            <i className="text-white text-xs mb-5">Ne vous en faites pas, ça arrive de perdre.</i>
            {status === "loading" && <Loader />}
            {error && <p className="text-white text-md mb-5">Erreur lors du chargement des stats...</p>}

            {games && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
                    <div className='flex flex-col w-full items-start font-semibold'>
                        <p className="text-white text-lg text-center">Nombre de parties jouées par mois</p>
                        <div ref={barChartRef} className="w-full h-80"></div>
                    </div>
                    <div className='flex flex-col w-full items-start font-semibold'>
                        <p className="text-white text-lg text-center">Répartition victoires / défaites</p>
